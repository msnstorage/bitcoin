...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string &strErrorRet)
{
    ...
    if(nBlockHeight < consensusParams.nSuperblockStartBlock) {
        ...
        if(nBlockHeight >= consensusParams.nBudgetPaymentsStartBlock &&
            ...
            if(masternodeSync.IsSynced() && !sporkManager.IsSporkActive(SPORK_5_OLD_SUPERBLOCK_FLAG)) {
...
bool IsBlockExist(int nBlockHeight, const std::shared_ptr<const CBlock> pblock)
{
    ...
    if(mnpayments.mapMasternodeBlocks.count(nBlockHeight)) {
      LogPrintf("CMasternodePayments::IsBlockExist -- successfully to detect block to pay:%d\n", nBlockHeight);
      return true;
    }
    LogPrintf("CMasternodePayments::IsBlockExist -- failed to detect block to pay:%d\n", nBlockHeight);
    return false;
...
int CMasternodePayments::GetMinMasternodePaymentsProto() {
    return sporkManager.IsSporkActive(SPORK_3_MASTERNODE_PAY_UPDATED_NODES)
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f BTC\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
void CMasternodePayments::Sync(CNode* pnode, CConnman& connman)
{
    ...
    for (int h = nCachedBlockHeight; h < nCachedBlockHeight + 10; h++) {
