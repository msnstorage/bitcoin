...
{
    CAmount Collateral = Params().GetConsensus().nMasternodeCollateral * COIN;

    return (TxValue == Collateral);
}

CAmount CMasternode::CollateralValue(int nHeight) {

    CAmount Collateral = Params().GetConsensus().nMasternodeCollateral;

    CAmount MnCollateral = Collateral * COIN;
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
    ...
    // always using first wallet with MN
